...
//looking for a user
	app.get("/users", function(req, res) {
		db.User.findOne({}).then(function(userData) {
			res.render("index", {username: userData});
		});
	});
// Display the user's trips
	app.get("/users/trips", function(req, res) {
		db.Trip.findAll({}).then(function(userTrips) {
			res.render("index", {location: userTrips});
		});
	});
// add a new trip
	app.post("users/trips/new-trip", function(req, res) {
		db.Trip.create({
			// The post will have days, start date and location. Arrange your object accoridingly
...
// Display days in a trip
	app.get("users/trips/daily-schedule", function(req, res) {
		db.Activity.findAll({}).then(function(schedule) {
			res.redirect("users");
		});
}
